...
(i.e. comment has been wrongly detected as spam by Akismet although it was OK)
Plugin URI: http://piwigo.org/ext/extension_view.php?eid=192
Author: Hood_fr
*/

function akismet_user_comment_submit_spam($comment_id, $where)
{
    ...
    if($comment['spam_feedback']=='ham')//if comment was tagged as ham, whereas it is actually spam, it is submited back to akismet as spam
    {
        ...
        {
        $answer=$akismet->submitSpam();
        }
        else{
        $answer='error in spam submission';
        }
    }
    else{
